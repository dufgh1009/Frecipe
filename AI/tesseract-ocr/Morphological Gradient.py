...
# kernel = np.ones((5, 5), np.uint8)
# erosion = cv2.erode(gray,kernel,iterations = 1) #축소
# dilation = cv2.dilate(gray,kernel,iterations= 1) #(확대)


# SCAN
# https://blog.naver.com/tommybee
...
config = "--psm 3"
text = pytesseract.image_to_string(closing, config=config, lang='kor')
# text = pytesseract.image_to_string(adaptive_threshold, config=config, lang='kor+eng')
# print(text)
# https://blog.naver.com/tommybee  Threshold.py
