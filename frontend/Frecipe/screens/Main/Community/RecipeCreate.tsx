...
class RecipeCreate extends Component<Props> {
  ...
  render() {
    ...
    const mainIngredientsInput = recipeAdd.mainIngredients.map((element: any) => {
    ...
    const sauceInput = recipeAdd.sauce.map((element: any) => {
      const index = element.id
      return <View key={element.id} style={{ flexDirection: 'row', borderColor: 'black', marginHorizontal: 5, height: 50 }}>
    ...
    const addList = recipeAdd.context.map((element: any) => {
      if (element.image === '') {
        return <View key={element.id} style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 10, height: 50, marginVertical: 10 }}>
          ...
          <Button buttonStyle={{ width: 60, marginHorizontal: 20 }} onPress={this.props.onCamera} type='clear' icon={<AntDesign name="pluscircleo" size={24} color="black" />}></Button>
        </View>
      } else {
        return <View key={element.id} style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 10, height: 50, marginVertical: 10 }}>
          <Text style={{ textAlignVertical: 'center', textAlign: 'center', width: 40 }}>{element.id + 1}</Text>
